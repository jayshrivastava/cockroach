...
// TestSettingsTraceSamplingRate tests that a batch size of 1
...
func TestSettingsZipkinBatchSize(t *testing.T) {
	...
	execCommand := func(command string) {
		if _, err := tc.ServerConn(rand.Intn(3)).Exec(command); err != nil {
			t.Fatal(err)
		}
	}

	// Configure tracing with a batch size of 1 in the test cluster
	execCommand(`SET CLUSTER SETTING trace.zipkin.collector.batch.size = 1`)
	execCommand(fmt.Sprintf(`SET CLUSTER SETTING trace.zipkin.collector = '%s'`, traceCollectorServerURL.Host))

	// Run commands to be traced
	execCommand(`CREATE DATABASE t`)
	execCommand(`CREATE TABLE test (k INT PRIMARY KEY, v INT)`)
	execCommand(`INSERT INTO test VALUES (1,1)`)
