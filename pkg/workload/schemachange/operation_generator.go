...
func (og *operationGenerator) randOp(tx *pgx.Tx) (string, string, error) {
	...
	for {
		...
		if err == nil {
			...
			// Check if the transaction is aborted. If so, return early to prevent an
			// infinite loop.
...
func (og *operationGenerator) addUniqueConstraint(tx *pgx.Tx) (string, error) {
	...
	constaintName := fmt.Sprintf("%s_%s_unique", tableName.Object(), column.name)

	columnExistsOnTable, err := columnExistsOnTable(tx, tableName, column.name)
	if err != nil {
		return "", err
	}
	...
	if columnExistsOnTable {
		canApplyConstraint, err = canApplyUniqueConstraint(tx, tableName, []string{column.name})
		if err != nil {
			return "", err
		}
	}
	...
	codesWithConditions{
		...
		{code: pgcode.FeatureNotSupported, condition: columnExistsOnTable && !colinfo.ColumnTypeIsIndexable(column.typ)},
	}.add(og.expectedExecErrors)
	...
	return fmt.Sprintf(`ALTER TABLE %s ADD CONSTRAINT %s UNIQUE (%s)`, tableName, constaintName, column.name), nil
}
